...
export default async function SlatePage() {
  ...
  return (
    <section style={{ display: "grid", gap: "1rem" }}>
      ...
      <article className="np-card" style={{ maxWidth: 520, margin: "0 auto", width: "100%", padding: "1rem", display: "grid", gap: "0.65rem" }}>
        {players.map((player, index) => (
          <div key={player.id} className="np-card-muted" style={{ padding: "0.8rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: "0.75rem" }}>
              <div>
                ...
                <div style={{ marginTop: 2, color: "var(--np-text-dim)", fontSize: "0.72rem" }}>
                  {player.team} · {player.matchupLabel}
                </div>
              </div>
